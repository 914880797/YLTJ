...
export async function previewAutoScore(env, targetDate) {
  ...
  if (!configs || configs.length === 0) {
    const { results: diag } = await env.DB.prepare(
      `SELECT id, name, bind_group_id FROM warmup_projects`
    ).all();
    return {
  ...
  for (const cfg of configs) {
    ...
    const label = `${cfg.warmup_project_name} > ${cfg.warmup_group_name} > ${cfg.bind_group_name}`;

    for (const name of rawNames) {
      if (name.startsWith('-')) {
        const realName = name.substring(1);
        const key = `${realName}::${cfg.warmup_project_id}`;
        if (!seenExcluded.has(key)) {
          seenExcluded.add(key);
          excludedList.push({ name: realName, source: label, warmup_project_id: cfg.warmup_project_id });
        }
      } else {
        const key = `${name}::${cfg.warmup_project_id}`;
...
export async function autoScoreWarmup(env, targetDate) {
  ...
  for (const cfg of (configs || [])) {
    ...
    const cacheKey = `${groupId}_warmupgroup_${cfg.warmup_group_id}`;
    if (!slotCache[cacheKey]) {
      if (cfg.has_slots === 0) {
        const slotName = `${cfg.warmup_project_name}_${cfg.warmup_group_name}`;
