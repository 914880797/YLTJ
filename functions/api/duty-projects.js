  ...
  try {
    const { results } = await env.DB.prepare(
      `SELECT dp.*, g.name as bind_group_name,
              (SELECT COUNT(*) FROM duty_groups WHERE duty_project_id = dp.id) as group_count
       FROM duty_projects dp
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO duty_projects (name, bind_group_id, order_index) VALUES (?, ?, ?)`
...
export async function onRequestPut({ request, env }) {
  ...
  try {
    ...
    if (bind_group_id !== undefined) {
      await env.DB.prepare(`UPDATE duty_projects SET bind_group_id = ? WHERE id = ?`)
